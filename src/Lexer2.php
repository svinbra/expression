...
class Lexer2
{
    ...
    /**
     * @return \Expression\Token2|null
     * @throws \Expression\CompilerError
     */
    public function eatToken(): ?Token2
    {
        $operator = false;
        ...
        if ($this->equals('++')) {
            if ($this->previousToken->type === Lexer2::LITERAL || $this->previousToken->type === Lexer2::VARIABLE) {
                $result->type = Lexer2::UNARY_POST_INCREMENT;
                $result->typeInfo = $this->typeInfo(Lexer2::UNARY_POST_INCREMENT);
                $operator = true;
            } elseif($this->isOperator($this->previousToken) || $this->previousToken === null) {
                $result->type = Lexer2::UNARY_PRE_INCREMENT;
                $result->typeInfo = $this->typeInfo(Lexer2::UNARY_PRE_INCREMENT);
                $operator = true;
            }
        } else {
        ...
        if (!$operator) {
            if ($this->equals(';')) {
                ...
                $result->type = Lexer2::VARIABLE;
                $result->string = substr($this->source, $start, $this->column - $start);
            } else {
                $result->type = Lexer2::UNKNOWN_TOKEN;
            }
        }
     ...
     * @throws \Expression\CompilerError
     */
    public function peekToken(): ?Token2
    {
    ...
    public function isOperator(?Token2 $token): bool
    {
       return $token && isset($this->operators[$token->type]);
    }

    public function typeInfo(int $type): array
    {
